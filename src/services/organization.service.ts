...
export const organizationService = {
  ...
  async get(id: string): Promise<OrganizationDetail> {
    const { data } = await api.get<ApiResponse<OrganizationDetail>>(`/organizations/${id}`);
    return data.data;
  },
  ...
  async create(payload: CreateOrganizationPayload): Promise<CreatedOrganization> {
    const { data } = await api.post<ApiResponse<CreatedOrganization>>("/organizations", payload);
    return data.data;
  },
  ...
  async update(id: string, payload: UpdateOrganizationPayload): Promise<Organization> {
    const { data } = await api.patch<ApiResponse<Organization>>(`/organizations/${id}`, payload);
    return data.data;
  },

  /** GET /organizations/:id/roles — `assignable` reflète ce que l'écriture acceptera. */
  async roles(id: string): Promise<OrganizationRole[]> {
    const { data } = await api.get<ApiResponse<OrganizationRole[]>>(`/organizations/${id}/roles`);
    return data.data;
  },
  ...
  async users(id: string, params: UserListParams = {}): Promise<Paginated<OrganizationUser>> {
    const { data } = await api.get<Paginated<OrganizationUser>>(`/organizations/${id}/users`, { params });
    return data;
  },
  ...
  async createUser(id: string, payload: CreateOrganizationUserPayload): Promise<CreatedUserAccount> {
    const { data } = await api.post<ApiResponse<CreatedUserAccount>>(`/organizations/${id}/users`, payload);
    return data.data;
  },
  ...
  async auditLogs(id: string, params: AuditLogListParams = {}): Promise<AuditLogPage> {
    const { data } = await api.get<AuditLogPage>(`/organizations/${id}/audit-logs`, { params });
    return data;
  },
};
